...
server.on("request",function(request,response){
	if(request.url.indexOf("/getAddress") == 0){
		...
		var priKey = querystring.parse(arg).priKey;         //name => a
		var toAddress = querystring.parse(arg).toAddress;         //name => a
		var amount = querystring.parse(arg).amount;         //name => a
		var memo = querystring.parse(arg).memo;         //name => a
